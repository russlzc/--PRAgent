    ...
    def do_GET(self) -> None:
        ...
        if path == "/github/install":
            if not self.settings.github_app_slug:
                self._send_json(503, {"error": "PRAGENT_GITHUB_APP_SLUG is not configured"})
                return
...
def run() -> None:
    ...
    print("PR Review Agent dashboard: http://%s:%d" % (settings.host, settings.port))
